from typing import cast

import torch
import torch.nn as nn
import torch.nn.functional as F

from mirrorshift.config import ModelConfig
from mirrorshift.modeling.attention import build_attention_block
from mirrorshift.modeling.decoder_blocks import DecoderBlock
from mirrorshift.modeling.ffn import FFN


def precompute_freqs_cis(
    dim: int,
    end: int,
    theta: float = 10000.0,
    device: torch.device | str | None = None,
) -> torch.Tensor:
    # theta variable is base theta, 10000 in original paper
    # theta ^ -2(i-1)/d
    freqs = 1.0 / (
        theta
        ** (torch.arange(0, dim, 2, device=device)[: (dim // 2)].float() / dim)
    )
    t = torch.arange(end, device=device)
    freqs = torch.outer(t, freqs)
    # imo more readable than ones_like
    freqs_cis = torch.polar(torch.ones(freqs.size(), device=device), freqs)
    # output dimensions are (end, dim//2)
    return freqs_cis


class CausalTransformer(nn.Module):
    def __init__(self, model_config: ModelConfig):
        # vocab size is equal for input and output
        super().__init__()
        self.model_config = model_config
        self.embedding_layer = nn.Embedding(
            model_config.vocab_size, model_config.embedding_dim
        )
        if model_config.attention_type == "gqa":
            rope_dim = model_config.embedding_dim // model_config.num_heads
        elif model_config.attention_type == "mla":
            rope_dim = cast(int, model_config.qk_rope_head_dim)
        else:
            raise ValueError(f"Unknown attention type: {model_config.attention_type}")
        self._rope_dim = rope_dim
        self.register_buffer("freqs_cis", torch.empty(0, dtype=torch.complex64))

        self.decoder_stack = nn.ModuleList(
            [
                DecoderBlock(
                    attention_block=build_attention_block(model_config),
                    ffn=FFN(
                        model_dim=model_config.embedding_dim,
                        feedforward_dim=model_config.feedforward_dim,
                    ),
                )
                for _ in range(model_config.num_layers)
            ]
        )
        self.lm_head = nn.Linear(
            model_config.embedding_dim, model_config.vocab_size, bias=False
        )
        if not self.embedding_layer.weight.is_meta:
            with torch.no_grad():
                self.init_weights()

    def init_weights(self) -> None:
        self.embedding_layer.reset_parameters()
        for layer in self.decoder_stack:
            layer.init_weights()
        self.lm_head.reset_parameters()
        nn.init.zeros_(self.lm_head.weight)
        self.freqs_cis = precompute_freqs_cis(
            self._rope_dim,
            self.model_config.context_length,
            device=self.embedding_layer.weight.device,
        )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x is batch size, seq_len where each value is a token index
        output = self.embedding_layer(x)
        output = F.rms_norm(output, (output.size(-1),))
        for layer in self.decoder_stack:
            output = layer(output, self.freqs_cis)
        output = F.rms_norm(output, (output.size(-1),))
        output = self.lm_head(output)
        return output
